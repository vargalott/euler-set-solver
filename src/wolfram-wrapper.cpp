...
System::String ^ __InvokingCLR::PrepareQuery::Evaluate(System::String ^ input) {
  ...
  for (int i = 0; i < input->Length; ++i) {
    if (i < input->Length - 1)
      if (System::Char::IsUpper(input[i]) &&
...
System::Drawing::Image ^
    __InvokingCLR::ResultImage::GetImage(System::String ^ url) {
...
System::Drawing::Bitmap ^
    __InvokingCLR::ResultImage::ResizeImage(System::Drawing::Image ^ image,
                                            int width) {
  System::Drawing::Bitmap ^ bitmap = gcnew System::Drawing::Bitmap(image);
  auto height = cli::safe_cast<int>(System::Math::Ceiling(
      cli::safe_cast<double>(bitmap->Height * width) / bitmap->Width));
  return gcnew System::Drawing::Bitmap(bitmap, width,
                                       (height / 2) + (width / 4));
}
void __InvokingCLR::ResultImage::PrintAscii(System::Drawing::Bitmap ^ bitmap) {
  ...
  for (int h = 0; h < bitmap->Height; h++) {
    for (int w = 0; w < bitmap->Width; w++) {
      ...
      System::Drawing::Color gray_color =
...
void __InvokingCLR::UseWolframAPI::SendRequest(System::String ^
                                               unevaluated_query) {
  System::String ^ query = PrepareQuery::Evaluate(unevaluated_query);

  Wolfram::Alpha::WolframAlphaService ^ service =
      gcnew Wolfram::Alpha::WolframAlphaService("");
  Wolfram::Alpha::Models::WolframAlphaRequest ^ request =
      gcnew Wolfram::Alpha::Models::WolframAlphaRequest{query};
...
System::String ^ __InvokingCLR::UseWolframAPI::GetVennUrl(void) {
  ...
  if (this->query_result->Result->QueryResult->Pods != nullptr)
    return this->query_result->Result->QueryResult->Pods[1]
        ->SubPods[0]
        ->Image->Src;
  return nullptr;
};
